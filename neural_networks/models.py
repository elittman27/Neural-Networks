...
from neural_networks.losses import initialize_loss, Loss
from neural_networks.optimizers import initialize_optimizer
from neural_networks.layers import initialize_layer, Layer
...
class NeuralNetwork(ABC):
    def __init__(
        ...
        self.loss: Loss = initialize_loss(loss)
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        dLdData = self.loss.backward(target, out)
        for i in range(self.n_layers -1, 0, -1): # Iterate through layers backwards
            layer = self.layers[i]
            dLdData = layer.backward(dLdData)
        return loss
